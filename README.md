...
- 离职交接：办理离职时须选择一名同店在职员工接手；自离职日起尚未打卡的班次、以及离职者担任的门店店长职务，在同一数据库事务内一次性转交并将员工置为离职；无合格接手人时窗口提示先调岗或换店，保证员工状态、班次、门店负责人不会只改一部分。
...
后端 `audit.middleware.ts` 会审计财务新增/修改/删除、员工新增/修改/删除、离职办理（`RESIGN_EMPLOYEE`，记录接手人与交接结果）、排班创建/自动排班/修改、门店新增/修改/删除等关键操作，记录到 `audit_logs` 表，字段包含 `operatorId`、`action`、`target`、`oldValue`、`newValue`、`ip`、`timestamp`。
